...
void mandelbrotSet(GWindow& gw, double minX, double incX,
    ...
    int width = gw.getCanvasHeight();
    int height = gw.getCanvasWidth();
    ...
    for (int i=0; i<pixels.numRows(); i++) {
        for (int j=0; j<pixels.numCols(); j++){
            ...
            if (mandelbrotSetIterations(c,maxIterations)==maxIterations)
            {
                pixels[i][j]=color;
            } else
            {
           //pixels[i][j]=palette[mandelbrotSetIterations(c,maxIterations)%palette.size()];
           pixels[i][j]=0;
            }
...
int mandelbrotSetIterations(Complex z, Complex c, int remainingIterations) {
    // TODO: write this function
    if ((z.abs()>4)){return remainingIterations;}
    else if (remainingIterations==0) {return 0;}
    mandelbrotSetIterations(z*z+c,c, remainingIterations-1);
